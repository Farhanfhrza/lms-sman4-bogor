    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border-t-4 border-[#1a6341] relative">
        <div class="p-6 text-gray-900">
            <div class="flex items-center mb-6 bg-gray-100 p-4 rounded-t-lg border-b-2 border-yellow-400">
                ...
                <h3 class="text-lg font-bold text-gray-800">Formulir Tambah Siswa Baru</h3>
            </div>

            <form method="POST" action="{{ route('admin.students.store') }}" enctype="multipart/form-data" class="max-w-2xl mx-auto space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Opsional. Otomatis dari NISN jika kosong.</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select id="enrollment_year" name="enrollment_year" required
                            ...
                            @for($y = date('Y') + 1; $y >= 2020; $y--)
                                <option value="{{ $y }}" {{ old('enrollment_year', date('Y')) == $y ? 'selected' : '' }}>{{ $y }}</option>
                            @endfor
                        </select>
