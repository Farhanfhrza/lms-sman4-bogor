<x-app-layout>
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Page Title -->
            <h1 class="text-3xl font-bold text-gray-900 mb-6">Kelas</h1>
            ...
            <div class="flex flex-col md:flex-row items-stretch md:items-center gap-3 mb-8">
                ...
                <!-- Search Form -->
                <form method="GET" action="{{ route('classes.index') }}" class="flex-1 md:max-w-xs" id="searchForm">
                ...
                <!-- Sort Dropdown -->
                <form method="GET" action="{{ route('classes.index') }}" class="md:w-auto">
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden cursor-pointer group transform hover:-translate-y-1 transition-transform">
                        <a href="{{ route('classes.show', $classSubject->id) }}" class="block h-full flex flex-col">
                            <!-- Thumbnail with dynamic gradient background -->
                            @php
                                $colors = ['blue', 'green', 'purple', 'orange', 'pink', 'indigo'];
                                $colorIndex = $classSubject->id % 6;
                            @endphp
                            ...
                            <div class="p-5 flex-1 flex flex-col items-center text-center">
                                ...
                                <p class="text-xs text-gray-400 mt-2">
                                    {{ $classSubject->teacher->user->full_name ?? 'Guru' }}
                                </p>
                                @endif
                            </div>
                        </a>
